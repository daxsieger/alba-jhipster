...
class RefugeeResourceIT {
    ...
    void putWithMissingIdPathParamRefugee() throws Exception {
        ...
        restRefugeeMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(refugee))
            )
    ...
    void deleteRefugee() throws Exception {
        ...
        // Delete the refugee
        restRefugeeMockMvc
            .perform(delete(ENTITY_API_URL_ID, refugee.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
